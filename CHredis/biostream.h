...
public:
	...
	bostream& write_std(const std::string& p,size_t len)
	{
		if(avail() < (long)len)
			throw biosexception(biosexception::overflow);

		memcpy(pcurr,p.c_str(),len);
		pcurr+=len;
...
public:
	...
	bistream& read_std(std::string& p,size_t len) {
		if(avail() < (long)len)
			throw biosexception(biosexception::end);
...
bostream& operator<<(bostream& bos, const std::vector<std::pair<T, P> >& _s){
	int size = (int)_s.size( );
	bos << size;

	typedef typename std::vector<std::pair<T, P> >::const_iterator vectorit;
	for ( vectorit it=_s.begin(); it!=_s.end(); ++it )
	{
		bos << it->first << it->second;
	}
	return bos;
}

template<class T>
bostream& operator<<(bostream& bos, const std::list<T>& _s){
	int size = (int)_s.size();
	bos << size;

	typedef typename std::list<T>::const_iterator listit;
	for (listit it = _s.begin(); it != _s.end(); ++it)
	{
		bos << (*it);
	}
	return bos;
...
bostream& operator<<(bostream& bos, const std::vector<T>& _s){
	...
	typedef typename std::vector<T>::const_iterator vectorit;
	for ( vectorit it=_s.begin(); it!=_s.end(); ++it )
	{
		bos << (*it);
	}
...
template<class T>
bostream& operator<<(bostream& bos, const std::set<T>& _s){
	int size = (int)_s.size( );
	bos << size;

	typedef typename std::set<T>::const_iterator setit;
	for ( setit it=_s.begin(); it!=_s.end(); ++it )
	{
		bos << (*it);
	}
	return bos;
...
template<class T, class P>
bostream& operator<<(bostream& bos, const std::map<T, P>& _s){
	int size = (int)_s.size( );
	bos << size;

	typedef typename std::map<T, P>::const_iterator mapit;
	for ( mapit it=_s.begin(); it!=_s.end(); ++it )
	{
		bos << (it->first) << (it->second);
	}
...
inline bistream& bistream::operator>>(std::string& _s){
	...
	operator>>(blen);
	if(blen < 0xff)
		return read_std(_s,blen);

	unsigned short wlen;
	operator>>(wlen);
	if(wlen < 0xfffe)
		return read_std(_s,wlen);

	unsigned long dwlen;
	operator>>(dwlen);
	return read_std(_s,dwlen);
}

template<class T, class P>
bistream& operator>>(bistream& bis, std::vector<std::pair<T,P> >& _s){
	int size = 0;
...
template<class T>
bistream& operator>>(bistream& bis, std::list<T>& _s){
	int size = 0;
...
template<class T>
bistream& operator>>(bistream& bis, std::set<T>& _s){
	int size = 0;
...
template<class T, class P>
bistream& operator>>(bistream& bis, std::map<T, P>& _s){
	int size = 0;
